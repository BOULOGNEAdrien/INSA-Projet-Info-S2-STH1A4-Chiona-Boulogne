...
public class Matrice {
    ...
    public class ResGauss{ 
		...
		int rang=0;                                 // sous class qui stock la signature et le rang
	 ...
	 public  ResGauss descenteGauss(){
		...
		imax =  this.lignePlusGrandPivot(e);   //identifie le plus grand pivot la la ligne
		 ...
		 if(imax != e && imax != -1){                                                        // si le pivot nest pas sur la diagonale on effectue une permutation
			...
			res.signature = res.signature+1;                                     // incremente la signature
		 }
	...
	for(int i =e+1 ;i<this.nbrLig;i++){  
		...
		if(this.coeffs[i][e] != 0.0){                //si le coeff de la ligne traité est non nul on l'annule avec tranvesction
			this.transvection(e,i);
		        res.rang = res.rang+1;                                         //incrementation du rang
		}
	...
	gg = gg.concatCo(Vecteur);
		 ...
		 System.out.println( gg.determinant() );
		  ...
		  gg.remonteeGauss();
           ...
           public Matrice systemResol(Matrice vecteur){
              
               if( vecteur.nbrCol != 1 || this.nbrCol != this.nbrLig ||this.nbrCol != vecteur.nbrLig ){
                  
                   throw new Error ("systemResol : taille incorrecte ");//ajouter une mise au carré 
               }
               
               
               Matrice M = this.concatCo(vecteur);
               System.out.println(M);
               double det = M.determinant();
              
               if(det==0){
                   throw new Error("determinant nul: systemResol");
               }
               M.remonteeGauss();
               System.out.println(M);
               Matrice res = M.subCols(M.nbrLig-1, M.nbrLig);
               
               return res;
           }
           
          public static void testSystemResol(){
               
          Matrice M = matAleatoirUnOuDeux(3,3,1/3);
          double[] vecteur = {1.0,2.0,3.0};
          
          System.out.println(M);
          Matrice res = M.systemResol(creeVecteur(vecteur));
